  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("sound/cnh.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load cnh.mp3")+Mix_GetError();

  startMusic();
  sounds.push_back( Mix_LoadWAV("sound/calvin_explode.wav") );
  sounds.push_back( Mix_LoadWAV("sound/moe_dies.wav") );
  sounds.push_back( Mix_LoadWAV("sound/cheer.wav") );
